...
energysystem.add(
    solph.Source(
        label="wastewater",
        outputs={bsld: solph.Flow(fix=data["wastewater"], nominal_value=10e5)},
    )
)
...
volume, conv_factor = bg_prod.compute()
print('Total volume of digester : ', volume)
print('biogas conversion factor: ', conv_factor)
...
energysystem.add(
    solph.Transformer(
        label="volatile_solids",
        inputs={bsld: solph.Flow()},
        outputs={bvol: solph.Flow(nominal_value=10e5)},
        conversion_factors={bvol: conv_factor},
    )
)
...
energysystem.add(
    solph.Transformer(
        label="digester_gas",
        inputs={bvol: solph.Flow()},
        outputs={bbgas: solph.Flow(nominal_value=10e5)},
        conversion_factors={bbgas: 0.14},
    )
)
...
energysystem.add(
    solph.Transformer(
        label="digester_slu",
        inputs={bsld: solph.Flow()},
        outputs={bslu: solph.Flow(nominal_value=10e4)},
        conversion_factors={bslu: 0.94},
    )
)
...
energysystem.add(
    solph.Transformer(
        label="CHP_th",
        inputs={bbgas: solph.Flow()},
        outputs={bheat: solph.Flow(nominal_value=10e4)},
...
print(
    results[(storage, None)]["sequences"]["2021-02-25 08:00:00":"2021-02-26 15:00:00"]
)
print("")
