...
bel = solph.Bus(label="electricity1")
bel_digester = solph.Bus(label= "electricity2")
...
# create heat bus
bheat = solph.Bus(label="heat1")
bheat_digester = solph.Bus(label="heat2")

# adding the buses to the energy system
energysystem.add(bsld, bel, bel_digester, bslu, beff1, beff2, bvol, bdig, bbgas, bch4, bheat, bheat_digester)
...
energysystem.add(
    solph.Sink(
        ...
        inputs={bel_digester: solph.Flow(fix=data["electricity_demand_digester"], nominal_value=1)},
    )
)
...
energysystem.add(
    solph.Sink(
        ...
        inputs={bheat_digester: solph.Flow(fix=data["heat_demand_digester"], nominal_value=1)},
    )
)
...
energysystem.add(
    solph.Transformer(
        ...
        inputs={bch4: solph.Flow()},
        outputs={bel_digester: solph.Flow(nominal_value=10e5, variable_costs=60)},
        conversion_factors={bel_digester: 0.70},
    )
)
...
energysystem.add(
    solph.Transformer(
        ...
        inputs={bch4: solph.Flow()},
        outputs={bheat_digester: solph.Flow(nominal_value=10e5, variable_costs=60)},
        conversion_factors={bheat_digester: 0.40},
    )
)
...
electricity1_bus = solph.views.node(results, "electricity1")
electricity2_bus = solph.views.node(results, "electricity2")
heat1_bus = solph.views.node(results, "heat1")
...
#
#     fig, ax = plt.subplots(figsize=(10, 5))
#     electricity1_bus["sequences"].plot(
#         ax=ax, kind="line", drawstyle="steps-post"
#     )
#     plt.legend(
...
print("********* Main results *********")
print(electricity1_bus["sequences"].sum(axis=0))
print(electricity2_bus["sequences"].sum(axis=0))
print(heat1_bus["sequences"].sum(axis=0))
...
print(inflow)
print(outflow)
# print(heat_bus["sequences"])

logging.info("*****Checking different parameters with WHO guideliness*******")
