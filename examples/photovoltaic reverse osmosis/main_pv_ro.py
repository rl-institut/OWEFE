...
os.chdir("../examples/photovoltaic reverse osmosis")
...
# define water source
source_water = pd.Series([1, 1, 1, 1, 1, 3, 5, 7, 12, 6, 4, 4, 9, 14, 8, 3, 4, 4, 9, 10, 6, 5, 3, 2],
                               index=date_time_index, name="source water")
...
# create water source bus
bws = solph.Bus(label="water source bus")

# create permeate bus
bp = solph.Bus(label="permeate bus")
# create brine bus
bb = solph.Bus (label="brine bus")
# add buses to the iWEFEs
energysystem.add(bsed, bedc, beac, bp, bb)
...
energysystem.add(
    solph.Source(
        label="water source",
        outputs={bws: solph.Flow(fix=source_water, nominal_value=1)})),
...
energysystem.add(
    solph.Transformer(
        ...
        inputs={bedc: solph.Flow()},
        outputs={beac: solph.Flow()},
        conversion_factors={bp: 0.3, bb: 0.7},
    )
)
...
# water production
energysystem.add(solph.Sink(label="water production", inputs={bp: solph.Flow()}))
# brine production
energysystem.add(solph.Sink(label="electricity production", inputs={bb: solph.Flow()}))
